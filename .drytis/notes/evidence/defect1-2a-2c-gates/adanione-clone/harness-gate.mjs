...
check('IR plan: cart-count NOT asserted (2b held — id-less counter policy)', !cartCountAsr, cartCountAsr ? `unexpected exp=${cartCountAsr.exp}` : 'none (expected)');
...
check('IR plan: cart-total NOT asserted (2b held)', !cartTotalAsr, cartTotalAsr ? `unexpected exp=${cartTotalAsr.exp}` : 'none (expected)');
...
if (exec) {
  ...
  // Re-anchored: assertion EVALUATION count is bounded by which steps executed;
// steps 7/8 fail pre-assertion (pacing), step-0006 evaluates 1. The derived-
// assertion count itself is pinned at IR level (≥3, entity-targeted).
...
const pacingSteps = new Set(['step-0007', 'step-0008']);
const notFoundNonPacing = notFoundErrs.filter(s => !pacingSteps.has(s.stepId ?? s.id ?? ''));
check('RUN_TEST: no ElementNotFound outside the documented pacing steps (Defect 1)', notFoundNonPacing.length === 0, JSON.stringify(notFoundNonPacing.map(s => ({ id: s.stepId ?? s.id, err: s.error?.message }))))
// DOCUMENTED (defect1-2a-2c spec §Out of scope): steps 7/8 (qty-plus after an
// immediately-following cart soft-nav whose consequence — the meal row — lands
// ~500ms AFTER navigation) fail on inter-step consequence PACING, not on
// locator resolution. Pin: the wait provably ran (durationMs ≈ 10s cap) —
// vs the baseline audit where the same steps failed in 2-5ms.
const qtySteps = (exec.stepResults ?? []).filter(s => String(s.error?.message ?? '').includes('Increase quantity of MEAL'));
const waitRan = qtySteps.every(s => (s.durationMs ?? 0) > 9000);
check('RUN_TEST: qty steps exercised the 10s wait (pacing gap documented, not resolution)', waitRan, JSON.stringify(qtySteps.map(s => ({ id: s.stepId ?? s.id, dur: Math.round(s.durationMs ?? 0) }))));
