...
// 2b hard pin: NO assertion (entity OR counter) may target #cart-root — the
// container's text is the concatenation of every counter; a textMatch against
// it is the wrong-element failure mode 2b/2c exist to remove.
check('IR plan: NO assertion targets #cart-root (wrong-element guard)', allAsr.every(a => (a.loc ?? '') !== '#cart-root'), JSON.stringify(allAsr.filter(a => (a.loc ?? '') === '#cart-root')));
...
check('IR plan: cart-count ASSERTED via verified attribute locator (2b POSITIVE)', !!cartCountAsr && /^\[data-(auto-id|test-id|test)="cart-count"\]$/.test(cartCountAsr.loc || ''), cartCountAsr ? `loc=${cartCountAsr.loc} exp=${cartCountAsr.exp}` : 'missing');
...
check('IR plan: cart-total ASSERTED via verified attribute locator, targeting the counter itself (2b POSITIVE + wrong-element fix)', !!cartTotalAsr && /^\[data-(auto-id|test-id|test)="cart-total"\]$/.test(cartTotalAsr.loc || ''), cartTotalAsr ? `loc=${cartTotalAsr.loc} exp=${cartTotalAsr.exp}` : 'missing');
