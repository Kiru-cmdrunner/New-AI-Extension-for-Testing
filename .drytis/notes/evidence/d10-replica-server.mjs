// Replica shop app for D2/D3 real-Chrome validation (same shape as D9 replica,
// plus mutable markup so the staleness path can be triggered deliberately).
import http from 'node:http';

const PORT = 8098;
const state = { cartBadgeClass: 'cart-badge-v1' }; // mutated by the staleness probe
...
<body>${body}<script>
// 1 Hz heartbeat noise (ambient timer ticks — same as D7/D9 replicas)
setInterval(()=>{window.__hb=(window.__hb||0)+1;},1000);
</script></body></html>`;

const routes = {
  '/': () => page("Bob's Shop", `<h1>Shop Replica</h1>
...
inp.addEventListener('input',()=>{
  ...
  '/search.html': (q) => page('Search "Bazaar"', `<h1>Results for "${q || ''}"</h1>
...
<a href="/">Continue shopping</a>`),
  '/__mutate': (v) => { state.cartBadgeClass = v || 'cart-badge-v2'; return 'mutated'; },
};

const srv = http.createServer((req, res) => {
  const u = new URL(req.url, `http://127.0.0.1:${PORT}`);
  console.log(`[srv] ${req.method} ${u.pathname}${u.search}`);
  if (u.pathname === '/__mutate') { routes['/__mutate'](u.searchParams.get('v')); res.writeHead(200); res.end('ok'); return; }
  ...
  if (handler) { res.writeHead(200, {'Content-Type':'text/html'}); res.end(handler(u.searchParams.get('q'), u.searchParams.get('id'))); return; }
