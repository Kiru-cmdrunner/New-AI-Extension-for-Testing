// Where does the destination resultingState land? S2-only, full interaction dump.
...
const chrome = spawn(CHROME_BIN, [`--remote-debugging-port=${PORT}`, '--user-data-dir=/tmp/p3-s2b-profile', '--load-extension=/workspace/dist', '--disable-extensions-except=/workspace/dist', '--headless=new', '--no-sandbox', '--disable-gpu', '--no-first-run', '--no-default-browser-check'], { stdio: 'ignore' });
...
await sleep(300);
await panelCmd('START_RECORDING');
await sleep(800);
...
await sleep(4000);
await panelCmd('STOP_RECORDING');
await sleep(2500);
const dump = await evalIn(panel, `(async () => {
  const all = await chrome.storage.local.get(null);
  const inter = all['cmdrunner_live_interactions'] || [];
  return inter.map((i) => ({
    type: i.type, id: i.interactionId, srcEv: i.triggerEvent?.eventId,
    winId: i.behavioralEvidence?.windowId, endReason: i.behavioralEvidence?.window?.endReason,
    rsItems: i.behavioralEvidence?.applicationEvidence?.resultingState?.items?.map((x) => x.kind + ':' + (x.entityId ?? x.domPath)) ?? null,
  }));
})()`);
