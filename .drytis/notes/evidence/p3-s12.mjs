...
async function main() {
  ...
  async function runScenario(id, path, actions, settleMs = 3500, postMs = 2500) {
    ...
    console.log('  [AK dump]', JSON.stringify({ ak: understanding?.applicationKnowledge, trans: (understanding?.transitions ?? []).map((t) => ({ changes: t.changes })) }).slice(0, 3000));
  ...
  // S1 — AJAX add-to-cart + delayed dialog
  {
    ...
    // Fix C: the counter is deduped by the counterPaths skip-guard (the
    // in-window counterChanges already record it), so a 'page-content'
    // change string is NOT the right signal. The D2→AK contract lands in
    // applicationKnowledge.counters as absolute values.
    const akCounters = understanding?.applicationKnowledge?.counters ?? {};
    const counterOk = Object.values(akCounters).some(
      (c) => Array.isArray(c.values) && c.values.some((v) => v.value === 1),
    );
    record('S1: D2→AK: counter lands in applicationKnowledge', counterOk, 'counters=' + JSON.stringify(Object.keys(akCounters)) + ' trans=' + trans.length);
  }
  ...
  // S2 — Vivo form-submit full reload
  {
    ...
    const navRs = navs.map((n) => n?.behavioralEvidence?.applicationEvidence?.resultingState ?? null).find(Boolean) ?? null;
    ...
    console.log('  [AK dump S2]', JSON.stringify({ ak: understanding?.applicationKnowledge, trans: (understanding?.transitions ?? []).map((t) => ({ changes: t.changes })) }).slice(0, 3000));
    const trans = understanding?.transitions ?? [];
    // Fix C: assert the destination state's ORDER entity landed in
    // applicationKnowledge.entities (absolute state), not change strings.
    const entities = understanding?.applicationKnowledge?.entities ?? {};
    const orderOk = Object.keys(entities).some((k) => k.includes('12345') || JSON.stringify(entities[k]).includes('12345'));
    record('S2: D2→AK destination entity in applicationKnowledge', orderOk, 'entities=' + JSON.stringify(Object.keys(entities)).slice(0, 120) + ' trans=' + trans.length);
  }


  const fails = results.filter((r) => !r.ok);
  console.log(`\n===== P3-S12 RESULTS: ${results.length - fails.length}/${results.length} PASS =====`);
