...
<main>
  ...
  <div id="toast-slot"><div id="live-toast" aria-live="polite" hidden>Saved successfully</div></div>
  <div id="snack-slot"><div id="live-snack" class="snackbar-msg" hidden>Filter applied</div></div>
  <aside id="log-slot"><div id="act-log" role="log">App ready</div></aside>
</main>
<script>
const delay = (ms) => new Promise((r) => setTimeout(r, ms));
const TICKETS = [
...
document.getElementById('btn-search').addEventListener('click', async () => {
  ...
  toast.hidden = false; toast.textContent = 'Showing ' + window.__lastCount + ' results';
  const snack = document.getElementById('live-snack');
  snack.hidden = false; snack.textContent = 'Filters applied';
  const logEl = document.getElementById('act-log');
  logEl.textContent = 'Search returned ' + window.__lastCount + ' tickets';
});
...
setTimeout(() => {
  document.getElementById('app').innerHTML = \`
    ...
    <div class="field">
      ...
      <input class="field-input" placeholder="Choose a date">
    </div>
    <div class="field">
    ...
    <p id="plan-badge" class="plan-state" data-state="idle">Idle</p>
    <div class="trips"></div>\`;
  wire();
}, 350);
...
function wire(){
  ...
  const dateEl = document.querySelector('.field-input[placeholder="Choose a date"]');
