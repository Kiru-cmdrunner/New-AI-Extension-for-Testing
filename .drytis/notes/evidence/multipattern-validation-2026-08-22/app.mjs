...
<main>
  ...
  <input id="q" name="q" placeholder="e.g. invoice">
...
function wire(){
  ...
  let t; originEl.addEventListener('input', () => {
    ...
    t = setTimeout(async () => {
      await delay(120);
      const m = CITIES.filter(c=>c.toLowerCase().startsWith(originEl.value.trim().toLowerCase()) && originEl.value.trim());
      listEl.innerHTML = m.map((c,i)=>'<li class="opt" data-idx="'+i+'">'+c+'</li>').join('');
      listEl.hidden = !m.length;
    }, 250);
  });
...
<body>
<header><strong>Corner Shop</strong><span class="cart-count" aria-label="Cart items">0</span><a data-route="/" class="nav-link">Desk</a></header>
<main>
  <h3>Cart</h3>
  <div class="products">
    <div class="card" data-sku="SKU-A"><div><strong>Notebook</strong><div class="muted">SKU-A</div></div>
      ...
      <button aria-label="Increase quantity of Notebook" class="step-up">+</button></div>
    </div>
