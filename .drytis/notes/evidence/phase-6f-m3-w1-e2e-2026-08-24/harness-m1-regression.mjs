...
const OUT = '/workspace/.drytis/notes/evidence/phase-6f-m3-w1-e2e-2026-08-24/m1-regression-dumps';
...
// 6F-M1 A (added 2026-08-23, gesture-ownership): the paired click after a
// cell mousedown that completed the DatePicker must be ABSORBED — no twin
// Click card on any react-datepicker__day cell, and no duplicate IR step.
...
check('6F-M1 zero twin Click cards on calendar cells',
  twinClicks.length === 0,
  `${twinClicks.length} twin clicks :: ${JSON.stringify(twinClicks.map(c => c.name))}`);
