...
async function main() {
  ...
  // Capture SW console output for diagnosis
  const swConsole = [];
  ws.on('message', (d) => {
    try {
      const m = JSON.parse(d);
      if (m.method === 'Runtime.consoleAPICalled' && m.sessionId === swS.sessionId) {
        swConsole.push({
          t: Date.now(),
          level: m.params.type,
  ...
  // Capture PAGE console (content script errors) around M4c
  const pageConsole = [];
  ws.on('message', (d) => {
    try {
      const m = JSON.parse(d);
      if (m.method === 'Runtime.consoleAPICalled' && m.sessionId === pageS.sessionId) {
        pageConsole.push({ level: m.params.type, text: (m.params.args ?? []).map((a) => a.value ?? a.description ?? '').join(' ').slice(0, 200) });
      }
      if (m.method === 'Runtime.exceptionThrown' && m.sessionId === pageS.sessionId) {
        pageConsole.push({ level: 'EXC', text: (m.params.exceptionDetails?.exception?.description ?? m.params.exceptionDetails?.text ?? '').slice(0, 300) });
      }
    } catch {}
  });
  ...
  await input('mouseMoved', { x: 5, y: 5 }); await sleep(500);
  await evalPage(`window.__carouselOn = false; true`);
  const preM4c = await evalSW(`(async () => JSON.stringify(await chrome.storage.local.get(null)))()`).then(JSON.parse).catch(() => null);
  s = await stopRecording();
  {
    const hv = hovers(s);
    const q = hq(hv[0] ?? {}) ?? null;
    ...
    if (preM4c) console.log(`[M4c-pre] liveHoverCards=${(preM4c['cmdrunner_live_interactions'] ?? []).filter((i) => i?.type === 'Hover').length} allCards=${(preM4c['cmdrunner_live_interactions'] ?? []).map((i) => i?.type).join(',')} ledgerRows=${(preM4c['cmdrunner_evidence_ledger'] ?? []).length} pending=${(preM4c['cmdrunner_pending_evidence'] ?? []).length}`);
    const preH = preM4c ? (preM4c['cmdrunner_live_interactions'] ?? []).filter((i) => i?.type === 'Hover') : [];
    if (preH.length) console.log(`[M4c-pre] hoverQ=${JSON.stringify(hq(preH[0]))}`);
    const preLedger = preM4c?.['cmdrunner_evidence_ledger'] ?? [];
    for (const row of preLedger.slice(-2)) {
      console.log(`[M4c-ledgerRaw] keys=${JSON.stringify(Object.keys(row ?? {}))} first200=${JSON.stringify(row).slice(0, 400)}`);
    }
    // Post-STOP: where did the evidence go? Any late-arriving qualification?
    const postKeys = Object.keys(s ?? {}).filter((k) => k.startsWith('cmdrunner_'));
    console.log(`[M4c-post] keys=${postKeys.join(',')} verify=${JSON.stringify(s?.['cmdrunner_verification_result'] ?? null).slice(0, 300)}`);
    const pend = s?.['cmdrunner_pending_evidence'] ?? [];
    console.log(`[M4c-post] pendingRows=${pend.length}`);
    for (const p of pend.slice(0, 3)) {
      const str = JSON.stringify(p);
      const qm = str.match(/"hoverQualification":\{[^}]*"verdict":"([^"]+)"/);
      console.log(`[M4c-post] pendingRow src=${p?.sourceEventType} q=${qm ? qm[1] : 'none'} len=${str.length}`);
    }
    const postLedgerAll = s?.['cmdrunner_evidence_ledger'] ?? [];
    for (const row of postLedgerAll.slice(-3)) {
      if (row?.eventType === 'mouseenter') console.log(`[M4c-postLedger] ${JSON.stringify(row).slice(0, 260)}`);
    }
    const allQ = JSON.stringify(s ?? {}).match(/"hoverQualification":\{"verdict":"([^"]+)"/g);
    console.log(`[M4c-post] allHoverQualifications=${JSON.stringify(allQ)}`);
    const buf = await evalPage(`(() => { try { const b = JSON.parse(sessionStorage.getItem('cmdrunner_evidence_buffer') || '[]'); return JSON.stringify(b.map((e) => ({ src: e.sourceEventType, q: e.hoverQualification ? e.hoverQualification.verdict : null, dom: (e.applicationEvidence?.domChanges ?? []).length }))); } catch (e2) { return 'ERR ' + e2.message; } })()`).catch((e2) => 'ERR ' + e2.message);
    console.log(`[M4c-pageBuffer] ${buf}`);
    console.log(`[M4c-pageConsole] ${pageConsole.length} entries:`);
    for (const e of pageConsole.slice(0, 25)) console.log(`  [pg] ${e.level} ${e.text}`);
    console.log('[M4c-SWconsole] last 30 entries around STOP:');
    for (const e of swConsole.slice(-30)) console.log(`  [sw] ${new Date(e.t).toISOString().slice(11, 23)} ${e.level} ${e.text}`);
  }
