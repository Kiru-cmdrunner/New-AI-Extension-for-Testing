...
import type { PlaywrightGeneratorOutput } from '../generation/contracts/generator-contract';
...
export class StorageService {
  ...
  // ── Generated Steps (Post-Stop Generation — B3) ──────────

  /**
   * Read canonical steps generated by the Generation Engine.
   * Returns an empty array when nothing is stored.
   */
  static async getGeneratedSteps(): Promise<TestStep[]> {
    const result = await chrome.storage.local.get(StorageKeys.GENERATED_STEPS);
    const stored = result[StorageKeys.GENERATED_STEPS];
    if (Array.isArray(stored)) {
      return stored;
    }
    return [];
  ...
  /**
   * Persist canonical steps generated by the Generation Engine.
   */
  static async setGeneratedSteps(steps: unknown[]): Promise<void> {
    await chrome.storage.local.set({ [StorageKeys.GENERATED_STEPS]: steps });
  }

  /**
   * Clear all generated steps.
   */
  static async clearGeneratedSteps(): Promise<void> {
    await chrome.storage.local.set({ [StorageKeys.GENERATED_STEPS]: [] });
  }

  // ── Generated Playwright (B6) ──────────────────────────

  /**
   * Read the generated Playwright test code.
   * Returns null when nothing is stored.
   */
  static async getGeneratedPlaywright(): Promise<{ testCode: string; isManualEdit: boolean; generatedAt: string } | null> {
    const result = await chrome.storage.local.get(StorageKeys.GENERATED_PLAYWRIGHT);
    const stored = result[StorageKeys.GENERATED_PLAYWRIGHT];
    if (stored && typeof stored === 'object' && 'testCode' in stored) {
      return stored as { testCode: string; isManualEdit: boolean; generatedAt: string };
    }
    return null;
  }

  /**
   * Persist the generated Playwright test.
   */
  static async setGeneratedPlaywright(data: PlaywrightGeneratorOutput): Promise<void> {
    await chrome.storage.local.set({ [StorageKeys.GENERATED_PLAYWRIGHT]: data });
  }

  /**
   * Clear the generated Playwright test.
   */
  static async clearGeneratedPlaywright(): Promise<void> {
    await chrome.storage.local.remove(StorageKeys.GENERATED_PLAYWRIGHT);
  }
