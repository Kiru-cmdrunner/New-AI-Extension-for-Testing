...
export class DexieUnitOfWork implements UnitOfWork {
  ...
  async execute<T>(work: (repos: RepositorySet) => Promise<T>): Promise<T> {
    // Dexie transaction across all tables that repositories might touch.
    // 'rw!' = read-write, all tables listed for optimal locking.
    // Using array form because Dexie's typed overloads cap at 5 explicit tables.
